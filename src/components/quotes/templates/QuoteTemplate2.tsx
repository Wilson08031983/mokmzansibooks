          ...
          <tbody>
            {displayData.items.map((item, i) => (
              <tr key={i} className={`${i % 2 === 0 ? 'bg-gray-50' : 'bg-white'} border-b border-gray-200`}>
                ...
                <td className="py-3 px-4 text-right">{formatCurrency(item.total || item.amount || 0)}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {displayData.bankAccount && (
        <div className="mt-8 bg-amber-50 rounded-lg p-4 border border-amber-200">
          <div className="flex items-center mb-2">
            <h3 className="font-bold text-amber-700 mb-0">Payment Details</h3>
            <Badge variant="bank" className="ml-2">Banking</Badge>
          </div>
          <div className="grid grid-cols-2 gap-4 mt-2">
            <div>
              <p className="text-sm text-gray-700"><span className="font-medium">Bank Name:</span> {displayData.bankAccount.bankName}</p>
              <p className="text-sm text-gray-700"><span className="font-medium">Account Name:</span> {displayData.bankAccount.accountName}</p>
            </div>
            <div>
              <p className="text-sm text-gray-700"><span className="font-medium">Account Number:</span> {displayData.bankAccount.accountNumber}</p>
              <p className="text-sm text-gray-700"><span className="font-medium">Branch Code:</span> {displayData.bankAccount.branchCode}</p>
              {displayData.bankAccount.swiftCode && (
                <p className="text-sm text-gray-700"><span className="font-medium">SWIFT Code:</span> {displayData.bankAccount.swiftCode}</p>
              )}
            </div>
          </div>
        </div>
