...
const InvoiceClassicTemplate = ({ data, preview }: TemplateProps) => {
  return (
    <div className="bg-white text-black font-serif">
      ...
      <div className="flex justify-between items-start mb-8">
        ...
        <div className="text-right">
          ...
          <p className="text-sm mt-2"><span className="font-semibold">Invoice #:</span> {data.invoiceNumber}</p>
          <p className="text-sm"><span className="font-semibold">Date:</span> {formatDate(data.issueDate)}</p>
          <p className="text-sm"><span className="font-semibold">Due Date:</span> {formatDate(data.dueDate)}</p>
      ...
      <table className="w-full mb-6">
        <thead>
          <tr className="text-left text-gray-800 border-b-2 border-gray-300">
            ...
            <th className="py-2 text-right">Rate (R)</th>
            <th className="py-2 text-right">Discount %</th>
            <th className="py-2 text-right">Amount (R)</th>
          </tr>
        </thead>
        <tbody>
          {data.items.map((item, index) => (
            <tr key={index} className="border-b border-gray-200">
              ...
              <td className="py-3 text-right">{formatCurrency(item.rate, "ZAR")}</td>
...
export default InvoiceClassicTemplate;
